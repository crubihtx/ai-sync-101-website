...
class AIDiscoveryWidget {
    ...
    async generatePDF() {
        ...
        const pageW = 210, pageH = 297, M = 16, CW = pageW - M * 2;
        const companyName = this.state.leadInfo?.company || null;
        ...
        if (!problemText) {
            ...
            problemText = firstUserMsg ? firstUserMsg.content : null;
        }
        ...
        const darkBg = () => { fill(BG); doc.rect(0, 0, pageW, pageH, 'F'); };

        // Full-width gradient bar
        const gradBar = (y, h) => {
            const seg = CW / 3;
            fill(BLUE);   doc.rect(M, y, seg, h, 'F');
            fill(PURPLE); doc.rect(M + seg, y, seg, h, 'F');
            fill(PINK);   doc.rect(M + seg*2, y, seg, h, 'F');
        };

        // Header with real logo
        const drawHeader = () => {
            // Logo image
            try { doc.addImage(LOGO_B64, 'PNG', M, 7, 38, 13); } catch(e) {
                // Fallback text if image fails
                doc.setFont('Inter','bold'); doc.setFontSize(12); text(WHITE);
                doc.setCharSpace(2); doc.text('AI SYNC 101', M, 15); doc.setCharSpace(0);
            }
            // Right label
            doc.setFont('Inter','normal'); doc.setFontSize(8); text(GRAY6);
            doc.text('DISCOVERY SESSION SUMMARY', pageW - M, 14, { align: 'right' });
            // Gradient rule
            gradBar(22, 0.6);
        };

        const drawFooter = () => {
            gradBar(pageH - 13, 0.4);
            doc.setFont('Inter','normal'); doc.setFontSize(7.5); text(GRAY6);
            doc.text('aisync101.com  ·  We build what you need, not what we sell', pageW/2, pageH - 7, { align: 'center' });
        };

        // Section heading (uppercase, tracked, colored)
        const heading = (label, y, color) => {
            doc.setFont('Inter','bold'); doc.setFontSize(6.5); text(color || GRAY6);
            doc.setCharSpace(2); doc.text(label.toUpperCase(), M, y); doc.setCharSpace(0);
            return y + 3.5;
        };

        // Card: dark bg + thin top border in accent color
        const card = (y, h, accent) => {
            fill(CARD); doc.rect(M, y, CW, h, 'F');
            draw(BORDER); doc.setLineWidth(0.1); doc.rect(M, y, CW, h, 'S');
            if (accent) { fill(accent); doc.rect(M, y, 2, h, 'F'); }
        };

        // Body text in card
        const bodyText = (str, x, y, maxW, lh, hex, size, style) => {
            doc.setFont('Inter', style||'normal'); doc.setFontSize(size||8.5); text(hex||'#CCCCCC');
            const lines = doc.splitTextToSize(str, maxW);
            doc.text(lines, x, y);
            return y + lines.length * lh;
        };

        // Measure wrapped line count
        const countLines = (str, maxW, size) => {
            doc.setFontSize(size||8.5);
            return doc.splitTextToSize(str, maxW).length;
        };

        // Numbered list in card
        const numList = (rawText, x, y, maxW, lh) => {
            const lines = rawText.split('\n').map(l => l.trim()).filter(l => l);
            let cy = y, n = 1;
            for (const line of lines) {
                const clean = line.replace(/^[\d]+[.)]\s*/,'').replace(/^[-*]\s*/,'');
                if (!clean) continue;
                doc.setFont('Inter','normal'); doc.setFontSize(8.5); text('#CCCCCC');
        ...
        const LH = 4.3, PAD = 5, GAP = 6, INNER = CW - 10;
        ...
        // ── Hero block: company name + date ──────────────────
        if (companyName) {
            doc.setFont('Inter','bold'); doc.setFontSize(18); text(WHITE);
        ...
        // Thin divider
        draw(BORDER); doc.setLineWidth(0.2);
        doc.line(M, y, M + CW, y);
        y += 7;

        // ── PROBLEM ──────────────────────────────────────────
        if (problemText) {
            y = heading('Problem Identified', y, BLUE);
            doc.setFontSize(8.5);
            const pLines = doc.splitTextToSize(problemText, INNER);
            const pH = pLines.length * LH + PAD * 2;
            card(y, pH, BLUE);
            bodyText(problemText, M + 6, y + PAD + 2, INNER, LH);
            y += pH + GAP;
        }

        // ── CURRENT WORKFLOW ─────────────────────────────────
        const currentText = workflow.current;
        if (currentText) {
            y = heading('Current Workflow', y, GRAY6);
            doc.setFontSize(8.5);
            let totalL = 0;
            currentText.split('\n').map(l=>l.trim()).filter(l=>l).forEach(l => {
                const cl = l.replace(/^[\d]+[.)]\s*/,'').replace(/^[-*]\s*/,'');
                totalL += countLines(cl ? `1. ${cl}` : l, INNER);
            });
            const cH = Math.max(totalL * LH + PAD * 2, 12);
            card(y, cH, GRAY6);
            numList(currentText, M + 6, y + PAD + 2, INNER, LH);
            y += cH + GAP;
        }

        // ── PROPOSED WORKFLOW ────────────────────────────────
        const proposedText = workflow.proposed;
        if (proposedText) {
            y = heading('Proposed Workflow', y, PINK);
            doc.setFontSize(8.5);
            let totalL = 0;
            proposedText.split('\n').map(l=>l.trim()).filter(l=>l).forEach(l => {
                const cl = l.replace(/^[\d]+[.)]\s*/,'').replace(/^[-*]\s*/,'');
            ...
            const pH2 = Math.max(totalL * LH + PAD * 2, 12);
            card(y, pH2, PINK);
            numList(proposedText, M + 6, y + PAD + 2, INNER, LH);
            y += pH2 + GAP;
        }

        // ── KEY CHANGE ───────────────────────────────────────
        if (workflow.keyChange) {
            y = heading('Key Change', y, PURPLE);
            doc.setFontSize(8.5);
            const kcLines = doc.splitTextToSize(workflow.keyChange, INNER);
            const kcH = kcLines.length * LH + PAD * 2;
            card(y, kcH, PURPLE);
            doc.setFont('Inter','bold'); doc.setFontSize(8.5); text(WHITE);
            doc.text(kcLines, M + 6, y + PAD + 2);
            y += kcH + GAP;
        }
        ...
        if (hasContact) {
            y = heading('Next Steps', y, BLUE);
            const contactParts = [visitorName, visitorEmail, visitorPhone, visitorWebsite].filter(Boolean);
            const nsH = 22;
            card(y, nsH, BLUE);

            // Left: contact
            doc.setFont('Inter','bold'); doc.setFontSize(6.5); text(GRAY6);
            doc.setCharSpace(1); doc.text('CONTACT', M + 6, y + 6); doc.setCharSpace(0);
            doc.setFont('Inter','normal'); doc.setFontSize(8); text(WHITE);
            doc.text(contactParts.join('  ·  '), M + 6, y + 11);

            // Right: CTA pill
            const ctaW = 54, ctaH = 8, ctaX = M + CW - ctaW - 4, ctaY = y + 7;
            fill(BLUE); doc.roundedRect(ctaX, ctaY, ctaW, ctaH, 1.5, 1.5, 'F');
            doc.setFont('Inter','bold'); doc.setFontSize(7.5); text(WHITE);
            doc.text('Discovery call within 24 hours', ctaX + ctaW/2, ctaY + 5.3, { align: 'center' });
        ...
        y = 30;
        doc.setFont('Inter','bold'); doc.setFontSize(6.5); text(GRAY6);
        doc.setCharSpace(2); doc.text('CONVERSATION TRANSCRIPT', M, y); doc.setCharSpace(0);
        y += 5;
        ...
        for (const msg of this.state.messages) {
            ...
            const bg = isUser ? '#111111' : '#181818';
            const accent = isUser ? BLUE : PINK;

            doc.setFont('Inter','normal'); doc.setFontSize(8.5);
            const tLines = doc.splitTextToSize(msg.content, CW - CPX * 2 - 2);
            const cardH = tLines.length * LH + CPY * 2 + 6;
            ...
            fill(bg); doc.rect(M, y, CW, cardH, 'F');
            // Left accent strip
            fill(accent); doc.rect(M, y, 1.5, cardH, 'F');

            doc.setFont('Inter','bold'); doc.setFontSize(6.5); text(accent);
            doc.setCharSpace(1); doc.text(label, M + CPX + 1, y + CPY + 2.5); doc.setCharSpace(0);

            doc.setFont('Inter','normal'); doc.setFontSize(8.5); text('#CCCCCC');
            doc.text(tLines, M + CPX + 1, y + CPY + 7);
