...
class AIDiscoveryWidget {
    ...
    async loadInterFonts(doc) {
        if (window._interFontsLoaded) return;
        const base = '/ai-discovery-widget/';
        const toB64 = (buf) => {
        ...
        const [regBuf, boldBuf] = await Promise.all([
            fetch(base + 'Inter-Regular.ttf').then(r => r.arrayBuffer()),
            fetch(base + 'Inter-Bold.ttf').then(r => r.arrayBuffer()),
        ]);
        const regB64  = toB64(regBuf);
        const boldB64 = toB64(boldBuf);
        doc.addFileToVFS('Inter-Regular.ttf', regB64);
        doc.addFileToVFS('Inter-Bold.ttf', boldB64);
        doc.addFont('Inter-Regular.ttf', 'Inter', 'normal');
        doc.addFont('Inter-Bold.ttf', 'Inter', 'bold');
        window._interFontsLoaded = true;
    }
    ...
    async generatePDF() {
        ...
        const company  = this.state.leadInfo?.company  || null;
        const name     = this.state.leadInfo?.name     || null;
        const email    = this.state.leadInfo?.email    || null;
        const phone    = this.state.leadInfo?.phone    || null;
        const website  = this.state.leadInfo?.website  || null;
        const dateStr  = new Date().toLocaleDateString('en-US', { year:'numeric', month:'long', day:'numeric' });
        const workflow = this.extractWorkflowFromMessages();

        let problem = this.state.leadInfo?.problem || null;
        if (!problem) {
            const m = this.state.messages.find(m => m.role === 'user' && m.content.length > 10);
            problem = m ? m.content : null;
        }

        const _rgb = h => [parseInt(h.slice(1,3),16), parseInt(h.slice(3,5),16), parseInt(h.slice(5,7),16)];
        const fc = h => { const [r,g,b]=_rgb(h); doc.setFillColor(r,g,b); };
        const tc = h => { const [r,g,b]=_rgb(h); doc.setTextColor(r,g,b); };
        const dc = h => { const [r,g,b]=_rgb(h); doc.setDrawColor(r,g,b); };

        const BG='#0A0A0A', CARD='#1A1A1A', BORDER='#2A2A2A';
        const BLUE='#00A3FF', PINK='#FF1F8F', PURPLE='#7B2FFF';
        const WHITE='#FFFFFF', GRAY3='#CCCCCC', GRAY4='#AAAAAA', GRAY5='#888888', GRAY6='#666666';

        const darkBg = () => { fc(BG); doc.rect(0,0,PW,PH,'F'); };

        const gradBar = (y, h=0.6) => {
            const s=CW/3;
            fc(BLUE);   doc.rect(M,    y, s, h, 'F');
            fc(PURPLE); doc.rect(M+s,  y, s, h, 'F');
            fc(PINK);   doc.rect(M+s*2,y, s, h, 'F');
        };

        const card = (y, h, accent) => {
            fc(CARD); doc.rect(M,y,CW,h,'F');
            dc(BORDER); doc.setLineWidth(0.5); doc.rect(M,y,CW,h,'S');
            if (accent) { fc(accent); doc.rect(M,y,1.4,h,'F'); }
        };

        const eyebrow = (label, y, color) => {
            doc.setFont('Inter','bold'); doc.setFontSize(6.5); tc(color||BLUE);
            doc.setCharSpace(1.2); doc.text(label.toUpperCase(), M, y); doc.setCharSpace(0);
            return y+4;
        };

        const lineCount = (str, maxW, sz) => {
            doc.setFontSize(sz||8.5);
            return doc.splitTextToSize(str, maxW).length;
        };

        const numList = (raw, x, y, maxW, lh) => {
            const items = raw.split('\n').map(l=>l.trim()).filter(l=>l);
            let cy=y, n=1;
            for (const item of items) {
                const clean = item.replace(/^[\d]+[.)]\s*/,'').replace(/^[-*]\s*/,'');
                if (!clean) continue;
                doc.setFont('Inter','normal'); doc.setFontSize(8.5); tc(GRAY4);
                const wrapped = doc.splitTextToSize(n+'. '+clean, maxW);
        ...
        const LH=4.3, PAD=5, GAP=7, INPX=M+6, INW=CW-9;

        const drawHeader = () => {
            try { doc.addImage(LOGO_B64,'PNG',M,6.5,38,13.1); } catch(e) {
                doc.setFont('Inter','bold'); doc.setFontSize(11); tc(WHITE);
                doc.setCharSpace(2); doc.text('AI SYNC 101',M,14); doc.setCharSpace(0);
            }
            doc.setFont('Inter','normal'); doc.setFontSize(7.5); tc(GRAY5);
            doc.text('Discovery Session Summary', PW-M, 14, { align:'right' });
            gradBar(21.5);
        };

        const drawFooter = () => {
            gradBar(PH-12, 0.4);
            doc.setFont('Inter','normal'); doc.setFontSize(7); tc(GRAY6);
            doc.text('aisync101.com  \xb7  We build what you need, not what we sell', PW/2, PH-7, { align:'center' });
        };
        ...
        if (company) {
            doc.setFont('Inter','bold'); doc.setFontSize(22); tc(WHITE);
            doc.text(company, M, y);
            y += 8;
        }
        doc.setFont('Inter','normal'); doc.setFontSize(8); tc(GRAY5);
        doc.text(dateStr, M, y);
        y += 5;
        dc(BORDER); doc.setLineWidth(0.3); doc.line(M,y,M+CW,y);
        y += 8;

        if (problem) {
            y = eyebrow('Problem Identified', y, BLUE);
            doc.setFontSize(8.5);
            const pL = doc.splitTextToSize(problem, INW);
            const pH = pL.length * LH + PAD*2;
            card(y, pH, BLUE);
            doc.setFont('Inter','normal'); doc.setFontSize(8.5); tc(GRAY4);
            doc.text(pL, INPX, y+PAD+2.5);
            y += pH+GAP;
        }

        if (workflow.current) {
            y = eyebrow('Current Workflow', y, GRAY5);
            let nL=0;
            workflow.current.split('\n').map(l=>l.trim()).filter(l=>l).forEach(l => {
                const c=l.replace(/^[\d]+[.)]\s*/,'').replace(/^[-*]\s*/,'');
                nL += lineCount(c ? '1. '+c : l, INW);
            });
            const cH = Math.max(nL*LH+PAD*2, 12);
            card(y, cH, GRAY5);
            numList(workflow.current, INPX, y+PAD+2.5, INW, LH);
            y += cH+GAP;
        }

        if (workflow.proposed) {
            y = eyebrow('Proposed Workflow', y, PINK);
            let nL=0;
            workflow.proposed.split('\n').map(l=>l.trim()).filter(l=>l).forEach(l => {
                const c=l.replace(/^[\d]+[.)]\s*/,'').replace(/^[-*]\s*/,'');
                nL += lineCount(c ? '1. '+c : l, INW);
            });
            const pH = Math.max(nL*LH+PAD*2, 12);
            card(y, pH, PINK);
            numList(workflow.proposed, INPX, y+PAD+2.5, INW, LH);
            y += pH+GAP;
        }

        if (workflow.keyChange) {
            y = eyebrow('Key Change', y, PURPLE);
            const kcL = doc.splitTextToSize(workflow.keyChange, INW);
            const kcH = kcL.length*LH+PAD*2;
            card(y, kcH, PURPLE);
            doc.setFont('Inter','bold'); doc.setFontSize(8.5); tc(WHITE);
            doc.text(kcL, INPX, y+PAD+2.5);
            y += kcH+GAP;
        }

        if (name || email || phone) {
            y = eyebrow('Next Steps', y, BLUE);
            const nsH = 24;
            card(y, nsH, BLUE);
            doc.setFont('Inter','bold'); doc.setFontSize(6); tc(GRAY5);
            doc.setCharSpace(0.8); doc.text('CONTACT', INPX, y+6); doc.setCharSpace(0);
            doc.setFont('Inter','normal'); doc.setFontSize(8); tc(GRAY3);
            const parts = [name,email,phone,website].filter(Boolean);
            doc.text(parts.join('  \xb7  '), INPX, y+11.5);
            const ctaW=56, ctaH=9, ctaX=M+CW-ctaW-4, ctaY=y+(nsH/2)-(ctaH/2);
            fc(BLUE); doc.roundedRect(ctaX, ctaY, ctaW, ctaH, 2, 2, 'F');
            doc.setFont('Inter','bold'); doc.setFontSize(7); tc(WHITE);
            doc.text('Discovery call within 24 hrs', ctaX+ctaW/2, ctaY+5.8, { align:'center' });
            y += nsH+GAP;
        }
        ...
        // PAGE 2+ — TRANSCRIPT
        doc.addPage(); darkBg(); drawHeader();
        y = 27;
        doc.setFont('Inter','bold'); doc.setFontSize(6.5); tc(GRAY5);
        doc.setCharSpace(1.5); doc.text('CONVERSATION TRANSCRIPT', M, y); doc.setCharSpace(0);
        y += 6;

        for (const msg of this.state.messages) {
            const isUser = msg.role === 'user';
            const label  = isUser ? 'YOU' : 'AI SYNC 101';
            const bgHex  = isUser ? '#111111' : '#171717';
            const accent = isUser ? BLUE : PINK;
            const txtCol = isUser ? GRAY3 : GRAY4;

            doc.setFont('Inter','normal'); doc.setFontSize(8.5);
            const tL = doc.splitTextToSize(msg.content, CW-12);
            const cH = tL.length*LH + 8*2 + 6;

            if (y+cH > PH-16) { drawFooter(); doc.addPage(); darkBg(); drawHeader(); y=27; }

            fc(bgHex); doc.rect(M, y, CW, cH, 'F');
            dc(BORDER); doc.setLineWidth(0.3); doc.rect(M, y, CW, cH, 'S');
            fc(accent); doc.rect(M, y, 1.4, cH, 'F');

            doc.setFont('Inter','bold'); doc.setFontSize(6.5); tc(accent);
            doc.setCharSpace(1); doc.text(label, M+6, y+6); doc.setCharSpace(0);

            doc.setFont('Inter','normal'); doc.setFontSize(8.5); tc(txtCol);
            doc.text(tL, M+6, y+12);
        ...
        const safeName = (company||'Session').replace(/[^a-zA-Z0-9]/g,'-');
