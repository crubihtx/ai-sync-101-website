    ...
    async generatePDF() {
        ...
        const drawHeader = () => {
            ...
            try { doc.addImage(LOGO_B64, 'PNG', M, 7, 38, 13); } catch(e) {
                ...
                doc.setFont('helvetica','bold'); doc.setFontSize(12); text(WHITE);
                doc.setCharSpace(2); doc.text('AI SYNC 101', M, 15); doc.setCharSpace(0);
            }
            // Right label
            doc.setFont('helvetica','normal'); doc.setFontSize(8); text(GRAY6);
        ...
        const drawFooter = () => {
            ...
            doc.setFont('helvetica','normal'); doc.setFontSize(7.5); text(GRAY6);
        ...
        const heading = (label, y, color) => {
            doc.setFont('helvetica','bold'); doc.setFontSize(6.5); text(color || GRAY6);
        ...
        const bodyText = (str, x, y, maxW, lh, hex, size, style) => {
            doc.setFont('helvetica', style||'normal'); doc.setFontSize(size||8.5); text(hex||'#CCCCCC');
        ...
        const numList = (rawText, x, y, maxW, lh) => {
            ...
            for (const line of lines) {
                ...
                if (!clean) continue;
                doc.setFont('helvetica','normal'); doc.setFontSize(8.5); text('#CCCCCC');
        ...
        if (companyName) {
            doc.setFont('helvetica','bold'); doc.setFontSize(18); text(WHITE);
        ...
        if (workflow.keyChange) {
            ...
            card(y, kcH, PURPLE);
            doc.setFont('helvetica','bold'); doc.setFontSize(8.5); text(WHITE);
        ...
        if (hasContact) {
            ...
            // Left: contact
            doc.setFont('helvetica','bold'); doc.setFontSize(6.5); text(GRAY6);
            doc.setCharSpace(1); doc.text('CONTACT', M + 6, y + 6); doc.setCharSpace(0);
            doc.setFont('helvetica','normal'); doc.setFontSize(8); text(WHITE);
            ...
            doc.setFont('helvetica','bold'); doc.setFontSize(7.5); text(WHITE);
        ...
        for (const msg of this.state.messages) {
            ...
            doc.setFont('helvetica','bold'); doc.setFontSize(6.5); text(accent);
